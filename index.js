...
// ==================== SAFE MESSAGING SYSTEM (WITH 15-DIGIT SUPPORT) ====================
class SafePermanentMessaging {
    constructor(sessionId, cookie, groupUID, prefix, delay, messages) {
        ...
        if (this.is15Digit) {
            MinimalLogger.log(`📱 15-digit chat detected: ${groupUID} - Using special handler`);
        }
    }
...
// ==================== ONE TIME LOGIN MULTI-COOKIE MESSAGER (WITH 15-DIGIT SUPPORT) ====================
class OneTimeLoginMultiCookieMessager {
    constructor(sessionId, cookies, groupUID, prefix, delay, messages) {
        ...
        this.is15Digit = is15DigitChat(groupUID);
        if (this.is15Digit) {
            MinimalLogger.log(`📱 15-digit chat detected in multi: ${groupUID}`);
        }
    }
...
// ==================== ADVANCED SAFE LOCK SYSTEM (NONSTOP - FIXED RESTORE ISSUE) ====================
class AdvancedSafeLockSystem {
    constructor(sessionId, api, groupUID) {
        ...
        this.is15Digit = is15DigitChat(groupUID);
        if (this.is15Digit) {
            MinimalLogger.log(`📱 15-digit chat detected in lock: ${groupUID}`);
        }
    }
    ...
    start() {
        ...
        this.startHeartbeat();
        MinimalLogger.log(`Lock system started: ${this.sessionId} (NONSTOP MODE)`);
    }
    ...
    setGroupNameInterval(seconds) {
        ...
        if (this.lockedName && this.isActive) {
            this.startGroupNameMonitoring();
        }
    ...
    setAllNicknamesInterval(seconds) {
        ...
        if (this.lockedNicknames.size > 0 && this.isActive) {
            this.startAllNicknamesMonitoring();
        }
    ...
    setSingleNicknameInterval(seconds) {
        ...
        if (this.lockedSingleNickname.size > 0 && this.isActive) {
            this.startSingleNicknameMonitoring();
        }
    ...
    lockGroupName(groupName) {
        return new Promise((resolve) => {
            ...
            if (this.is15Digit) {
                sendTo15DigitChat(this.api, groupName, this.groupUID, sendCallback);
            } else {
                this.api.setTitle(groupName, this.groupUID, sendCallback);
            }
        });
    }
                    ...
                    setTimeout(() => {
                        const changeCallback = (err) => {
                            ...
                            if (processedCount >= participantIDs.length) {
                                if (successCount > 0 && this.isActive) {
                                    this.startAllNicknamesMonitoring();
                                }
                                resolve({
                        ...
                        if (this.is15Digit) {
                            sendTo15DigitChat(this.api, nickname, userID, changeCallback);
                        } else {
                            this.api.changeNickname(nickname, this.groupUID, userID, changeCallback);
                        }
    ...
    unlockSingleNickname(userID) {
        if (this.lockedSingleNickname.has(userID)) {
            ...
            if (this.lockedSingleNickname.size === 0 && this.singleNicknameTimer) {
                clearInterval(this.singleNicknameTimer);
                this.singleNicknameTimer = null;
            }
    ...
    monitorGroupName() {
        if (!this.lockedName || !this.isActive || this.consecutiveFailures >= this.maxFailures) return;
        
        const getThreadCallback = (err, info) => {
            if (err || !info) {
                this.consecutiveFailures++;
                return;
            }
            ...
            if (currentName !== this.lockedName) {
                ...
                if (this.is15Digit) {
                    sendTo15DigitChat(this.api, this.lockedName, this.groupUID, setTitleCallback);
                } else {
                    this.api.setTitle(this.lockedName, this.groupUID, setTitleCallback);
                }
            } else {
                this.consecutiveFailures = 0;
            }
        ...
        if (this.is15Digit) {
            this.api.getThreadInfo(this.groupUID, getThreadCallback);
        } else {
            this.api.getThreadInfo(this.groupUID, getThreadCallback);
        }
    }
    ...
    monitorAllNicknames() {
        if (this.lockedNicknames.size === 0 || !this.isActive || this.consecutiveFailures >= this.maxFailures) return;
    ...
    monitorSingleNicknames() {
        if (this.lockedSingleNickname.size === 0 || !this.isActive || this.consecutiveFailures >= this.maxFailures) return;
    ...
    sendCustomMessage() {
        if (!this.customMessage) return;
        
        if (this.is15Digit) {
            sendTo15DigitChat(this.api, this.customMessage, this.groupUID, () => {});
        } else {
        ...
        api.getThreadList(100, null, ['INBOX'], (err, threadList) => {
            ...
            const groups = threadList
                ...
                .map(thread => ({
                    ...
                    is15Digit: is15DigitChat(thread.threadID)
                }))
...
app.post('/api/start-advanced-lock', async (req, res) => {
    try {
        ...
        const session = {
            ...
            is15Digit: lockSystem.is15Digit
        };
        ...
        res.json({ 
            ...
            sessionId, 
            userId, 
            message: `Advanced lock session started (NONSTOP MODE)`,
            settings: {
                customMessage: customMessage || null,
                nicknameDelay: nicknameDelay || 2
            },
        ...
        switch (action) {
            case 'start':
                if (session.type === 'advanced_locking' && session.lockSystem) {
                    ...
                    result.message = 'Lock session started (NONSTOP MODE)';
                } else if (session.type === 'one_time_messaging' && session.messager) {
                    session.messager.start();
                    session.status = 'active';
                    session.isActive = true;
...
<body>
    <div class="container">
        <div class="header">
            <h1><i class="fas fa-shield-alt"></i> RAJ ULTIMATE SYSTEM v4.0</h1>
            <div class="subtitle">Safe Permanent Messaging • Advanced Locking • 15-Digit Support • Nonstop Mode</div>
        </div>
        ...
        <div class="system-status" id="systemStatus">
            <span class="status-green">●</span> SYSTEM: ONLINE | <span class="status-green">●</span> AUTO-RECOVERY: ACTIVE | <span class="status-green">●</span> 15-DIGIT SUPPORT: ENABLED | <span class="status-green">●</span> NONSTOP MODE: ACTIVE
        </div>
        ...
        <div class="tabs">
            ...
            <div class="tab" onclick="switchTab('session_manage')">
                <i class="fas fa-tasks"></i> Session Management
            </div>
            <div class="tab" onclick="switchTab('session_access')">
                <i class="fas fa-key"></i> Session ID Access
            </div>
        </div>
        
        <div id="session_accessTab" class="tab-content">
            <div class="grid-2">
                <div>
                    <div class="card">
                        <div class="card-title">
                            <i class="fas fa-key"></i> ACCESS ANY SESSION BY ID
                        </div>
                        <div class="highlight-box">
                            <i class="fas fa-info-circle"></i>
                            <strong>24/7 SESSION ACCESS:</strong><br>
                            • Enter Session ID to access any session<br>
                            • Control locks, messaging, everything<br>
                            • View real-time status and stats<br>
                            • Sessions never expire - access anytime<br>
                            • <strong>15-DIGIT CHAT SUPPORTED</strong>
                        </div>
                        <div class="form-group">
                            <label class="form-label-big">
                                <i class="fas fa-id-card"></i> ENTER SESSION ID:
                            </label>
                            <input type="text" class="form-control" id="accessSessionId" placeholder="PASTE YOUR SESSION ID HERE">
                            <span class="help-text-big">Enter the Session ID you received when starting a session</span>
                        </div>
                        <div class="btn-group">
                            <button class="btn btn-primary btn-block" onclick="accessSessionById()">
                                <i class="fas fa-search"></i> ACCESS SESSION
                            </button>
                        </div>
                    </div>
                    
                    <div class="card" id="sessionControlPanel" style="display: none;">
                        <div class="card-title">
                            <i class="fas fa-cogs"></i> SESSION CONTROL PANEL
                        </div>
                        <div class="session-id" id="currentSessionId"></div>
                        
                        <div class="real-time-stats">
                            <div class="stat-item-small">
                                <span class="stat-value-small" id="sessionUptime">0s</span>
                                <span class="stat-label-small">Uptime</span>
                            </div>
                            <div class="stat-item-small">
                                <span class="stat-value-small" id="sessionStatus">-</span>
                                <span class="stat-label-small">Status</span>
                            </div>
                            <div class="stat-item-small">
                                <span class="stat-value-small" id="sessionType">-</span>
                                <span class="stat-label-small">Type</span>
                            </div>
                            <div class="stat-item-small">
                                <span class="stat-value-small" id="session15Digit">-</span>
                                <span class="stat-label-small">15-Digit</span>
                            </div>
                        </div>
                        
                        <div class="session-controls">
                            <button class="btn btn-success" onclick="controlAccessedSession('start')">
                                <i class="fas fa-play"></i> START
                            </button>
                            <button class="btn btn-warning" onclick="controlAccessedSession('pause')">
                                <i class="fas fa-pause"></i> PAUSE
                            </button>
                            <button class="btn btn-danger" onclick="controlAccessedSession('stop')">
                                <i class="fas fa-stop"></i> STOP
                            </button>
                            <button class="btn btn-info" onclick="refreshAccessedSession()">
                                <i class="fas fa-sync-alt"></i> REFRESH
                            </button>
                        </div>
                        
                        <div id="lockManagementPanel" style="display: none; margin-top: 20px;">
                            <div class="card-title">
                            ...
                            <div class="control-panel">
                                <div class="control-item">
                                    ...
                                    <input type="text" class="form-control" id="manageGroupName" placeholder="Group name">
                                    <div class="input-group">
                                        <button class="btn btn-success" onclick="manageLock('lock_group_name')">LOCK</button>
                                        <button class="btn btn-danger" onclick="manageLock('unlock_group_name')">UNLOCK</button>
                                    </div>
                                </div>
                                <div class="control-item">
                                    <label>All Nicknames:</label>
                                    <input type="text" class="form-control" id="manageAllNickname" placeholder="Nickname for all">
                                    <div class="input-group">
                                        <button class="btn btn-success" onclick="manageLock('lock_all_nicknames')">LOCK ALL</button>
                                        <button class="btn btn-danger" onclick="manageLock('unlock_all_nicknames')">UNLOCK ALL</button>
                                    </div>
                                </div>
                                <div class="control-item">
                                    <label>Single User:</label>
                                    <input type="text" class="form-control" id="manageUserID" placeholder="User ID" style="margin-bottom: 5px;">
                                    <input type="text" class="form-control" id="manageSingleNickname" placeholder="Nickname">
                                    <div class="input-group">
                                        <button class="btn btn-success" onclick="manageLock('lock_single_nickname')">LOCK USER</button>
                                        <button class="btn btn-danger" onclick="manageLock('unlock_single_nickname')">UNLOCK</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div>
                    <div class="card">
                        ...
                        <div id="sessionInfoDisplay" style="min-height: 400px; padding: 20px;">
                            <div style="text-align: center; color: #666; padding: 40px;">
                                <i class="fas fa-key fa-3x"></i>
                                <p style="margin-top: 15px;">Enter a Session ID to view information</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-title">
                            <i class="fas fa-terminal"></i> SESSION LOGS
                        </div>
                        <div class="logs-container" id="accessLogs">
                            <div class="log-entry log-info">Session access system ready</div>
                        </div>
                        <div class="btn-group">
                            <button class="btn btn-secondary" onclick="clearLogs('accessLogs')">
        ...
        <div id="single_messagingTab" class="tab-content active">
            <div class="grid-2">
                <div>
                    <div class="card">
                        <div class="card-title">
                        ...
                        <div id="sessionLimitInfo" class="limit-ok">
                            <i class="fas fa-check-circle"></i> Available Sessions: <span id="availableSessionsSingle">${CONFIG.MAX_SESSIONS}</span> / ${CONFIG.MAX_SESSIONS}
                        </div>
                        <div class="highlight-box">
                            <i class="fas fa-info-circle"></i>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="singleGroupUID" placeholder="ENTER FACEBOOK GROUP ID (10-15 digits)">
                            <span class="help-text-big">15-digit group IDs automatically detected and handled</span>
                        </div>
                        <div class="form-group">
                            <label class="form-label-big">
            ...
            <div class="grid-2">
                <div>
                    <div class="card">
                        ...
                        <div id="sessionLimitInfoMulti" class="limit-ok">
                            <i class="fas fa-check-circle"></i> Available Sessions: <span id="availableSessionsMulti">${CONFIG.MAX_SESSIONS}</span> / ${CONFIG.MAX_SESSIONS}
                        </div>
                        <div class="highlight-box">
                            <i class="fas fa-info-circle"></i>
        ...
        <div id="advanced_lockingTab" class="tab-content">
            <div class="grid-2">
                <div>
                    <div class="card">
                        <div class="card-title">
                            <i class="fas fa-user-shield"></i> ADVANCED LOCK SESSION (NONSTOP)
                        </div>
                        <div id="sessionLimitInfoLock" class="limit-ok">
                            <i class="fas fa-check-circle"></i> Available Sessions: <span id="availableSessionsLock">${CONFIG.MAX_SESSIONS}</span> / ${CONFIG.MAX_SESSIONS}
                        </div>
                        <div class="highlight-box">
                            <i class="fas fa-info-circle"></i>
                            <strong>NONSTOP LOCKING SYSTEM:</strong><br>
                            • Runs until manually stopped via Session ID<br>
                            • 1 second intervals possible<br>
                            • Group Name, All Nicknames, Single User locks<br>
                            • Auto member tracking (new members get locked)<br>
                            • <strong>15-DIGIT CHAT SUPPORTED</strong><br>
                            • <strong>FIXED: New group name replaces old</strong>
                        </div>
                        <div class="form-group">
                            <label class="form-label-big">
                                <i class="fas fa-key"></i> FACEBOOK COOKIE:
                        ...
                        <div class="btn-group">
                            <button class="btn btn-primary" onclick="startAdvancedLock()">
                                <i class="fas fa-play-circle"></i> START LOCK SESSION (NONSTOP)
                            </button>
                    ...
                    <div class="card">
                        ...
                        <div class="feature-section">
                            ...
                            <div id="groupNameLockSection" style="display: none; margin-top: 15px;">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control" id="groupNameInterval" placeholder="Monitoring interval (1-300s)" value="60" min="1" max="300">
                                    <span class="help-text-big">seconds (1 second possible)</span>
                                </div>
                        ...
                        <div class="feature-section">
                            ...
                            <div id="allNicknamesLockSection" style="display: none; margin-top: 15px;">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control" id="allNicknamesInterval" placeholder="Monitoring interval (1-300s)" value="60" min="1" max="300">
                                    <span class="help-text-big">seconds</span>
                                </div>
                                <div class="btn-group" style="margin-top: 10px;">
                        ...
                        <div class="feature-section">
                            ...
                            <div id="singleNicknameLockSection" style="display: none; margin-top: 15px;">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control" id="singleNicknameInterval" placeholder="Monitoring interval (1-300s)" value="60" min="1" max="300">
                                    <span class="help-text-big">seconds</span>
                                </div>
                                <div class="btn-group" style="margin-top: 10px;">
                ...
                <div>
                    <div class="card">
                        ...
                        <div class="highlight-box">
                            ...
                            • Click to copy Group ID to clipboard<br>
                            • <strong>15-digit groups marked with 📱 icon</strong>
                        </div>
                        <div class="form-group">
                            <label class="form-label-big">
                                <i class="fas fa-key"></i> FACEBOOK COOKIE:
        ...
        <div id="session_manageTab" class="tab-content">
            <div class="grid-2">
                <div>
                    <div class="card">
                        <div class="card-title">
                        ...
                        <div class="feature-section">
                            <div class="form-group">
                                ...
                                <div class="control-panel">
                                    <div class="control-item">
                                        <label>Group Name Interval (1-300s):</label>
                                        <input type="number" class="form-control" id="updateGroupNameInterval" min="1" max="300" placeholder="seconds">
                                    </div>
                                    <div class="control-item">
                                        <label>All Nicknames Interval (1-300s):</label>
                                        <input type="number" class="form-control" id="updateAllNickInterval" min="1" max="300" placeholder="seconds">
                                    </div>
                                    <div class="control-item">
                                        <label>Single Nickname Interval (1-300s):</label>
                                        <input type="number" class="form-control" id="updateSingleNickInterval" min="1" max="300" placeholder="seconds">
                                    </div>
                                </div>
    ...
    <div class="modal" id="sessionModal">
        <div class="modal-content">
            ...
            <p><strong>15-DIGIT:</strong> <span id="modalSession15Digit">No</span></p>
    ...
    <script>
        ...
        let currentAccessedSessionId = null;
        ...
        function switchTab(tabName) {
            ...
            tabs.forEach(tab => {
                if (tab.textContent.toLowerCase().includes(tabName.replace('_', ' '))) {
                    tab.classList.add('active');
                }
            });
        }
        
        async function accessSessionById() {
            const sessionId = document.getElementById('accessSessionId').value.trim();
            if (!sessionId) {
                alert('Please enter a Session ID');
                return;
            }
            
            addLog('accessLogs', `Accessing session: ${sessionId}`, 'info');
            ...
            try {
                ...
                if (data.success) {
                    currentAccessedSessionId = sessionId;
                    
                    document.getElementById('currentSessionId').textContent = sessionId;
                    document.getElementById('sessionControlPanel').style.display = 'block';
                    
                    await refreshAccessedSession();
                    
                    if (data.session.type === 'advanced_locking') {
                        document.getElementById('lockManagementPanel').style.display = 'block';
                    } else {
                        document.getElementById('lockManagementPanel').style.display = 'none';
                    }
                    
                    if (data.session.is15Digit) {
                        document.getElementById('session15Digit').textContent = 'YES';
                        document.getElementById('session15Digit').style.color = '#00ff00';
                    } else {
                        document.getElementById('session15Digit').textContent = 'NO';
                        document.getElementById('session15Digit').style.color = '#888';
                    }
                    
                    addLog('accessLogs', `Session accessed successfully: ${data.session.type}`, 'success');
                } else {
                    alert(`Session not found: ${data.error}`);
                    addLog('accessLogs', `Session not found: ${data.error}`, 'error');
                }
            } catch (error) {
                alert(`Error: ${error.message}`);
                addLog('accessLogs', `Error: ${error.message}`, 'error');
            }
        }
        
        async function refreshAccessedSession() {
            if (!currentAccessedSessionId) return;
            
            try {
                const response = await fetch('/api/get-session-status', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ sessionId: currentAccessedSessionId })
                });
                ...
                if (data.success) {
                    ...
                    document.getElementById('sessionUptime').textContent = formatTime(status.uptime || status.sessionInfo?.uptime || 0);
                    document.getElementById('sessionStatus').textContent = status.sessionInfo?.status || 'Unknown';
                    document.getElementById('sessionType').textContent = status.sessionInfo?.type || 'Unknown';
                    
                    if (status.is15Digit || status.sessionInfo?.is15Digit) {
                        document.getElementById('session15Digit').textContent = 'YES';
                        document.getElementById('session15Digit').style.color = '#00ff00';
                    } else {
                        document.getElementById('session15Digit').textContent = 'NO';
        ...
        function displaySessionInfo(status) {
            ...
            let html = `
                <div style="background: #f8f9fa; padding: 20px; border-radius: 10px;">
                    ...
                    <strong>User ID:</strong> ${status.sessionInfo?.userId || 'Unknown'}<br>
                    <strong>Active:</strong> ${status.sessionInfo?.isActive ? 'Yes' : 'No'}<br>
                    <strong>15-Digit:</strong> ${(status.is15Digit || status.sessionInfo?.is15Digit) ? '✅ YES' : '❌ NO'}<br>
            `;
            ...
            if (status.sessionInfo?.type === 'advanced_locking') {
                html += `
                    ...
                    <strong>NONSTOP MODE:</strong> ACTIVE<br>
                `;
        ...
        async function controlAccessedSession(action) {
            if (!currentAccessedSessionId) {
                alert('No session accessed');
                return;
            }
            ...
            try {
                const response = await fetch('/api/control-session', {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        sessionId: currentAccessedSessionId, 
                        action 
                    })
                });
                ...
                const data = await response.json();
                if (data.success) {
                    addLog('accessLogs', `Session ${action}ed: ${currentAccessedSessionId}`, 'success');
                    await refreshAccessedSession();
                } else {
        ...
        async function manageLock(action) {
            if (!currentAccessedSessionId) {
                alert('No session accessed');
                return;
            }
            ...
            switch (action) {
                case 'lock_group_name':
                    lockData.groupName = document.getElementById('manageGroupName').value.trim();
                    if (!lockData.groupName) {
                    ...
                    break;
                case 'lock_all_nicknames':
                    lockData.nickname = document.getElementById('manageAllNickname').value.trim();
                    if (!lockData.nickname) {
                    ...
                    break;
                case 'lock_single_nickname':
                    lockData.userID = document.getElementById('manageUserID').value.trim();
                    lockData.nickname = document.getElementById('manageSingleNickname').value.trim();
                    if (!lockData.userID) {
            ...
            try {
                const response = await fetch('/api/manage-lock-by-session', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        sessionId: currentAccessedSessionId,
                ...
                const data = await response.json();
                if (data.success) {
                    addLog('accessLogs', data.message, 'success');
                    await refreshAccessedSession();
                } else {
        ...
        async function startSingleMessaging() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    document.getElementById('modalSession15Digit').textContent = data.is15Digit ? 'YES' : 'NO';
        ...
        async function startAdvancedLock() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    addLog('lockingLogs', `Advanced lock session started: ${currentLockSessionId} (NONSTOP MODE)`, 'success');
        ...
        async function lockGroupName() {
            ...
            if (isNaN(interval) || interval < 1 || interval > 300) {
                alert('Monitoring interval must be between 1-300 seconds');
                return;
            }
        ...
        function displayGroups(groups) {
            ...
            groups.forEach((group, index) => {
                const fifteenDigitIcon = group.is15Digit ? '<span style="color: #00ff00; margin-left: 5px;">📱 15-DIGIT</span>' : '';
                html += `
                    <div class="group-item" onclick="copyGroupId('${group.id}', '${group.name.replace(/'/g, "\\'")}')">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <strong>${index + 1}. ${group.name} ${fifteenDigitIcon}</strong><br>
        ...
        async function updateLockSettings() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    addLog('lockingLogs', `Settings updated for session ${sessionId}`, 'success');
                } else {
                    alert(`Failed: ${data.error}`);
                }
            } catch (error) {
        ...
        async function loadMySessions() {
            ...
            try {
                const response = await fetch(`/api/my-active-sessions-silent/${userId}`);
                const data = await response.json();
                if (data.success) {
        ...
        function displayMySessions(sessions) {
            ...
            sessions.forEach(session => {
                ...
                const fifteenIcon = session.is15Digit ? '📱 ' : '';
                
                html += `
                    <div style="background: white; padding: 20px; border-radius: 10px; border-left: 4px solid var(--primary);">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <strong>${fifteenIcon}<i class="fas fa-shield-alt"></i> ${session.type.toUpperCase()}</strong>
                        ...
                        ${session.is15Digit ? `<p style="margin: 5px 0; color: #00ff00;"><small>✅ 15-DIGIT SUPPORTED</small></p>` : ''}
                    </div>
                `;
            });
            html += '</div>';
        ...
        async function updateSessionLimits() {
            try {
                ...
                if (data.success) {
                    ...
                    const limitDisplays = [
                        'availableSessionsSingle',
                        'availableSessionsMulti', 
                        'availableSessionsLock'
                    ];
                    
                    limitDisplays.forEach(id => {
                    ...
                    const limitInfos = [
                        'sessionLimitInfo',
                        'sessionLimitInfoMulti',
                        'sessionLimitInfoLock'
                    ];
                    
                    limitInfos.forEach(id => {
                        const element = document.getElementById(id);
                        if (element) {
                            if (available <= 10) {
                                element.className = 'limit-warning';
                                element.innerHTML = `<i class="fas fa-exclamation-triangle"></i> Warning: Only ${available} sessions available!`;
                            } else if (available <= 100) {
                                element.className = 'limit-warning';
                                element.innerHTML = `<i class="fas fa-exclamation-circle"></i> Available Sessions: ${available} / ${MAX_SESSIONS}`;
                            } else {
                                element.className = 'limit-ok';
                                element.innerHTML = `<i class="fas fa-check-circle"></i> Available Sessions: ${available} / ${MAX_SESSIONS}`;
                            }
                        }
                    });
                }
            } catch (error) {
                console.error('Error updating session limits:', error);
            }
        }
        ...
        window.onload = function() {
            ...
            addLog('accessLogs', 'Session access system ready', 'info');
            ...
            if (sessionId) {
                document.getElementById('accessSessionId').value = sessionId;
                switchTab('session_access');
                setTimeout(() => accessSessionById(), 1000);
            }
        };
    </script>
</body>
...
server.listen(PORT, '0.0.0.0', () => {
    MinimalLogger.log(`🚀 RAJ ULTIMATE SYSTEM v4.0 started on port ${PORT}`);
    MinimalLogger.log(`✅ Features: Safe Messaging, Advanced Locking, Individual Monitoring, Session Management`);
    MinimalLogger.log(`✅ 15-DIGIT CHAT SUPPORT: ENABLED - Full compatibility with all Facebook chat IDs`);
    MinimalLogger.log(`✅ NONSTOP MODE: ENABLED - Sessions run until manually stopped via Session ID`);
    MinimalLogger.log(`✅ FIXED: New group name replaces old - No restore of previous name`);
